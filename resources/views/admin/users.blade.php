 ...
 <table class="table table-responsive offset-2">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class= "text-center">{{$user->pseudo}} - {{$user->prenom}} {{$user->nom}}</td>
        <td class= "text-center">{{$user->email}}</td>
        <td class= "text-center">{{$user->groupe}}</td>
        <td class= "text-center">{{$user->isAdmin}}</td>
        <td class= "text-center">{{$user->isActif}}</td>
        <td class= "text-center">{{$user->created_at}}</td>
        <td class= "text-center">
            <a href = "/pronostic/admin/user/{{$user->id}}/edit" ><i class="btn btn-info   fas fa-search"></i></a>
