...
*/
        ...
        foreach($matchs as $match){
            if (!array_key_exists($match->pseudo, $tableau_point)){
                ...
                $tableau_point[$match->pseudo]['bon_ecart'] = 0;
                $tableau_point[$match->pseudo]['bon_score'] = 0;
                $tableau_point[$match->pseudo]['nom'] = $match->nom;
                $tableau_point[$match->pseudo]['prenom'] = $match->prenom;
            }
            ...
            if ( in_array($match->phase,['tour1', 'tour2','tour3']))
                ...
                && ($match->pronostic1 != NULL || $match->pronostic2 != NULL ))
                {
                    ...
                    || ($match->resultat1 <> $match->pronostic1 && $match->resultat2 == $match->pronostic2)){
                        ...
                        $tableau_point[$match->pseudo]['bon_score']++;
                    }
                    else if (($match->resultat1 <> $match->pronostic1 && $match->resultat2 <> $match->pronostic2) &&
                        ($match->resultat1 - $match->resultat2 == $match->pronostic1 - $match->pronostic2)){
                        $tableau_point[$match->pseudo]['points'] =$tableau_point[$match->pseudo]['points']+  6*$multiplicateur + $points_8eme;
                        $tableau_point[$match->pseudo]['bon_ecart']++;
                    }
                    else if ($match->resultat1 == $match->pronostic1 && $match->resultat2 == $match->pronostic2){
        ...
        foreach ($tableau_point as $key => $row) {
            ...
            $bon_score[$key] = $row['bon_score'];
            $bon_ecart[$key] = $row['bon_ecart'];
        }
        ...
        foreach($matchs as $match){
                ...
                && ($match->pronostic1 != NULL || $match->pronostic2 != NULL ))
            {
                if (($match->resultat1 <> $match->pronostic1 && $match->resultat2 <> $match->pronostic2) &&
                    ...
                    ($match->resultat1 - $match->resultat2 == $match->pronostic1 - $match->pronostic2))
                    $tableau_prono[$i]['points']=  6*$multiplicateur + $points_8eme;
                else if ($match->resultat1 == $match->pronostic1 && $match->resultat2 == $match->pronostic2)
